    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            if (cause.getMessage() != null) {
                ...
                if (cause instanceof SourceLocationBound) {
                    SourceLocationBound loc = (SourceLocationBound) cause;
                    System.err.println("       line(position): "
                            + (loc.getLineNumber() + 1) + "(" + loc.getPosition()
                            + ")");
                }
            } else {
                throw sce;
