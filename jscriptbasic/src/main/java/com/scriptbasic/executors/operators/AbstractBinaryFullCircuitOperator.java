 ...
 * 
 * @author Peter Verhas date May 31, 2012
 */
public abstract class AbstractBinaryFullCircuitOperator extends
        AbstractBinaryOperator {
    protected abstract RightValue evaluateOn(ExtendedInterpreter interpreter,
            RightValue leftOperand, RightValue rightOperand)
            throws BasicRuntimeException;

    @Override
    public final RightValue evaluate(ExtendedInterpreter interpreter)
            throws ExecutionException {
        return evaluateOn(interpreter, getLeftOperand().evaluate(interpreter),
                getRightOperand().evaluate(interpreter));
    }
}
